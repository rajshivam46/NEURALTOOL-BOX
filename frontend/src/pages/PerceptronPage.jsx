...
import { UploadCloud, CheckCircle, AlertCircle, Zap, Cpu } from 'lucide-react';
import MetricsChart from '../components/MetricsChart';
import { uploadData, trainModel, predictModel } from '../api';

const PerceptronPage = ({ sessionId, setSessionId }) => {
  ...
  const handleTrain = async () => {
    try {
      ...
      const res = await trainModel('perceptron', {
        session_id: sessionId,
        target_col: targetCol, feature_cols: featureCols,
        lr: parseFloat(lr), epochs: parseInt(epochs)
      });
  ...
  const handlePredict = async () => {
    try {
      const res = await predictModel('perceptron', {
        session_id: sessionId,
        feature_values: featureCols.map(f => parseFloat(predictInputs[f]))
      });
  ...
  return (
    <div className="animate-fade-in">
      <div className="page-header">
        <h2 className="page-title">Perceptron (Binary Classifier)</h2>
        <p className="page-subtitle">Upload a dataset and train a single-layer perceptron to classify binary data.</p>
      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2">
              <div className="form-group">
                ...
                <select className="form-control" value={targetCol} onChange={e => setTargetCol(e.target.value)}>
                  {data.columns.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div className="form-group">
                <label className="form-label">Features (X)</label>
                <div style={{maxHeight: '150px', overflowY: 'auto', border: '1px solid var(--border-color)', borderRadius: 'var(--radius)', padding: '0.5rem'}}>
                  {data.columns.filter(c => c !== targetCol).map(c => (
                    <div key={c} style={{display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem'}}>
                       <input type="checkbox" checked={featureCols.includes(c)} onChange={() => handleFeatureToggle(c)} /> <span>{c}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {data && (
        <div className="card">
          ...
          <div className="grid grid-cols-2" style={{maxWidth: '500px'}}>
          ...
          <button className="btn btn-primary mt-4" onClick={handleTrain} disabled={trainingStatus === 'training'}>
            {trainingStatus === 'training' ? <div className="spinner"></div> : <Zap size={20} />}
            {trainingStatus === 'training' ? 'Training...' : 'Train Perceptron Model'}
          </button>
        </div>
      )}

      {metrics && <MetricsChart errors={metrics.errors} accuracies={metrics.accuracies} labels={['Errors (Misclassifications)', 'Accuracy']} />}
          ...
          <div className="grid grid-cols-4">
            {featureCols.map(f => (
              <div className="form-group" key={f}>
                ...
                <input type="number" step="any" className="form-control" value={predictInputs[f] || 0} onChange={e => setPredictInputs({...predictInputs, [f]: e.target.value})} />
              </div>
            ))}
          </div>
